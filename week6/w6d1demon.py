...
#displays on row of data from the file
display(16, len(name)
                                             )
...
while ans == "y":
    ...
    if search_type not in ["1", "2", "3", "4"]:
        ...
        if search not in ["dragon", "elf"]:
            print("Sorry, 'dragon' or 'elf' are accepted> Please try again.")
            ...
            #display results
            if not found: #if the found is still empty
                print(f"Sorry, your search for {search} came up empty :C")
            else:
        ...
        if not found:
            ...
            print(f"Huzzah! we found your search for {search}, see details below")
            display("x", len(found))
