...
while ans == "y":
    ...
    if choice != "1" and choice != "2" and choice != "3" and choice != "4": 
        ...
        print("\n~Search by NAME~")
          #BUBBLE SORT -- "always sort before the binary search"
        for i in range(len(names) - 1):
            for j in range(len(names)- 1):
                if colors[j] > names[j + 1]:
                    #SWAP!
                    swap(j, colors)
                    swap(j, names)
                    swap(j,nums)
        ...
        if search.lower() == names[mid].lower():
            #FOUND IT!
            print(f"Wefound youe search for {search}, details below: ")
            print(f"{'Name':12}   {'NUM':3}   {'COLOR':10}")
            print(f"-----------------------------------------------------")
            print(f"{names[mid]:12}   {nums[mid]:3}   {colors[mid]:10}")
            print(f"-----------------------------------------------------")
        else:
            print(f"\nYour search for {search} is complete, no macthes found :C\n")
...
#-------------2D LIST--------------------------
#2D lists are just lists that contain that 1D list insde of them!
print("2D LIST------------------------------------------------------------")
dataFile = []
with open("text_files\simple.csv") as csvfile:
        file = csv.reader(csvfile)
