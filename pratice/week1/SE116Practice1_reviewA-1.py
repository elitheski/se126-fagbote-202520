...
def again(): #<- FUNCTION HEADER
    ...
    '''this function asks a user if they'd  like to enter another temp, checks the response for validity, and then returns a valid respone back to the main program'''
    ans = input("\t\tWould you like to enter another temperature? [y/n]:").lower()
    
    #user error trap loop - ensures user provides valid value
    while ans != "y" and answer != "n":
        print("***INVALID ENTRY***")
        ans = input("\t\tWould you like to enter another temperature? [y/n]:").lower()
...
while answer == "y" or answer == "Y":
    ...
    answer = again() #returns value will repalce this function call and store 'answer'
